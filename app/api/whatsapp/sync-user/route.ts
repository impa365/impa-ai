...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📊 Encontradas ${connections.length} conexões para sincronizar`)
    ...
    for (const connection of connections) {
      try {
        console.log(`🔄 Sincronizando: ${connection.connection_name} (${connection.instance_name})`)
        ...
        try {
          ...
          if (statusResponse.ok) {
            ...
            if (statusData?.instance?.state) {
              switch (statusData.instance.state) {
                case "open":
                  newStatus = "connected"
                  phoneNumber = statusData.instance?.wuid || statusData.instance?.number || phoneNumber
                  break
                case "connecting":
          ...
          if (fetchError.name === "AbortError") {
            console.warn(`⏱️ Timeout ao verificar status da instância ${connection.instance_name}`)
          } else {
            console.warn(`⚠️ Erro de rede ao verificar instância ${connection.instance_name}:`, fetchError.message)
          }
        }
        ...
        if (newStatus !== connection.status || phoneNumber !== connection.phone_number) {
          ...
          const updateResponse = await fetch(`${supabaseUrl}/rest/v1/whatsapp_connections?id=eq.${connection.id}`, {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
              "Accept-Profile": "impaai",
          ...
          if (updateResponse.ok) {
            console.log(`✅ Conexão ${connection.connection_name} atualizada: ${connection.status} → ${newStatus}`)
            syncedCount++
            results.push({
            ...
            console.error(`❌ Erro ao atualizar conexão ${connection.connection_name}`)
            results.push({
              connectionName: connection.connection_name,
              success: false,
          ...
          console.log(`ℹ️ Conexão ${connection.connection_name} já está atualizada`)
          syncedCount++
          results.push({
            connectionName: connection.connection_name,
            success: true,
        ...
        console.error(`💥 Erro ao sincronizar conexão ${connection.connection_name}:`, error)
        results.push({
          connectionName: connection.connection_name,
          success: false,
    ...
    console.log(`✅ Sincronização do usuário concluída: ${syncedCount}/${connections.length} conexões`)
