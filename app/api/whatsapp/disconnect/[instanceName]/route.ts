...
export async function DELETE(request: NextRequest, { params }: { params: { instanceName: string } }) {
  try {
    ...
    const logoutResponse = await fetch(`${config.apiUrl}/instance/logout/${instanceName}`, {
      method: "DELETE",
      headers: {
    ...
    if (!logoutResponse.ok) {
      ...
      return NextResponse.json(
        { success: false, error: `Erro ao desconectar: ${logoutResponse.status}` },
        { status: 500 },
      )
    }
    ...
    const updateResponse = await fetch(`${supabaseUrl}/rest/v1/whatsapp_connections?instance_name=eq.${instanceName}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        "Accept-Profile": "impaai",
    ...
    if (!updateResponse.ok) {
      console.error("Erro ao atualizar status no banco, mas desconexão foi bem-sucedida")
    }
    ...
    if (error.name === "TimeoutError") {
      return NextResponse.json({ success: false, error: "Timeout ao desconectar instância" }, { status: 408 })
    }
