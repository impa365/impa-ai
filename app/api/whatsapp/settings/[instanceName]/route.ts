...
export async function GET(
  ...
  try {
    ...
    // Tentar buscar configurações da Evolution API
    try {
...
export async function POST(
  ...
  try {
    ...
    // Verificar se o usuário tem permissão para esta instância
    let url = `${supabaseUrl}/rest/v1/whatsapp_connections?instance_name=eq.${instanceName}`
    if (user.role !== "admin") {
      url += `&user_id=eq.${user.id}`
    }
