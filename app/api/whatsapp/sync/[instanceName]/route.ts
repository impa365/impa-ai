...
export async function POST(request: NextRequest, { params }: { params: { instanceName: string } }) {
  try {
    ...
    if (!updateResponse.ok) {
      return NextResponse.json({ success: false, error: "Erro ao atualizar status no banco" }, { status: 500 })
    }
    ...
    if (error.name === "TimeoutError") {
      return NextResponse.json(
        { success: false, error: "Timeout ao verificar status na Evolution API" },
        { status: 408 },
      )
    }
