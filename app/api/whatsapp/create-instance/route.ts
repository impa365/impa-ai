  ...
  try {
    ...
    if (!connectionName || !userId) {
      return NextResponse.json(
        { success: false, error: "Nome da conexão e ID do usuário são obrigatórios" },
        { status: 400 },
      )
    }
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: "URL da Evolution API inválida na configuração" },
        { status: 400 },
      )
    }
    ...
    do {
      ...
      if (attempts > 10) {
        return NextResponse.json(
          { success: false, error: "Erro ao gerar identificadores únicos. Tente novamente." },
          { status: 500 },
        )
      }
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: "Resposta da Evolution API não é um JSON válido." },
        { status: 500 },
      )
    }
    ...
    if (!evolutionData || (!Array.isArray(evolutionData) && !evolutionData.instance)) {
      ...
      return NextResponse.json(
        { success: false, error: "Resposta da Evolution API tem formato inesperado." },
        { status: 500 },
      )
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        evolutionResponse: Array.isArray(evolutionData) ? evolutionData[0] : evolutionData,
      },
    })
  } catch (error: any) {
