...
export async function GET(request: NextRequest) {
  try {
    ...
    if (dia) {
      const [day, month, year] = dia.split("/")
      const filterDate = new Date(`${year}-${month}-${day}`)
      countQuery = countQuery.eq("start_date", filterDate.toISOString().split("T")[0])
    }
    ...
    const formattedLeads = leads?.map((lead) => ({
      ...lead,
      start_date_formatted: new Date(lead.start_date).toLocaleDateString("pt-BR"),
      days_since_start: Math.floor(
        (new Date().getTime() - new Date(lead.start_date).getTime()) / (1000 * 60 * 60 * 24),
      ),
