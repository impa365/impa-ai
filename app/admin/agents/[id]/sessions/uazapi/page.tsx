...
export default function UazapiAgentSessionsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("🔍 [UAZAPI SESSIONS] Dados do agente:", {
      ...
      // 🚨 VERIFICAR TIPO DE API - Apenas Uazapi usa esta página
      const apiType = agentData.agent?.connection?.api_type
      
      if (apiType === 'evolution') {
        console.log('⚠️ [UAZAPI SESSIONS] Agente é Evolution, redirecionando...')
        toast({
          title: "Tipo de API Incorreto",
          description: "Esta página é exclusiva para agentes Uazapi. Redirecionando...",
          variant: "destructive"
        })
        // Redirecionar para a página correta de sessões Evolution
        router.push(`/admin/agents/${agentId}/sessions`)
        return
      }

      // ✅ É Uazapi, buscar sessões da tabela bot_sessions
  ...
  const fetchSessions = async (agentData?: any) => {
    try {
      ...
      // 🔒 SEGURANÇA: Validar que o agente tem bot_id
      if (!currentAgent?.bot_id) {
        console.error("❌ SEGURANÇA: Agente sem bot_id!")
        toast({
          title: "Erro de Configuração",
          description: "Este agente não possui bot_id configurado. Não é possível buscar sessões.",
          variant: "destructive",
        })
        setSessions([])
        return
      ...
      console.log("🔍 Buscando sessões Uazapi do bot:", currentAgent.bot_id)
  ...
  const handleToggleStatus = async (session: BotSession) => {
    try {
      ...
      toast({
        title: "Sucesso",
        description: `Sessão ${newStatus ? "ativada" : "pausada"} com sucesso`,
      })
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🚀 Sessões do Bot Uazapi</h1>
