  ...
  try {
    ...
    if (!integrationData?.config?.apiUrl || !integrationData?.config?.apiKey) {
      return {
        success: false,
        error:
          "Evolution API não configurada. Entre em contato com o administrador.",
      };
    }
    ...
    if (
      !evolutionResponse ||
      (!Array.isArray(evolutionResponse) && !evolutionResponse.instance)
    ) {
      console.error(
    ...
    return {
      ...
      data: {
        ...
        evolutionResponse: Array.isArray(evolutionResponse)
          ? evolutionResponse[0]
          : evolutionResponse,
      },
    };
...
export async function fetchInstanceDetails(instanceName: string): Promise<{
  ...
  try {
    ...
    if (!isJsonResponse(response)) {
      const responseText = await response.text();
      console.error(
        "Evolution API retornou resposta não-JSON ao buscar instâncias:",
        {
          status: response.status,
          responsePreview: responseText.substring(0, 200) + "...",
        }
    ...
    const instanceDetails = Array.isArray(data)
      ? data.find((instance) => instance.name === instanceName)
      : data;
...
export async function getInstanceQRCode(instanceName: string): Promise<{
  ...
  try {
    ...
    if (!isJsonResponse(response)) {
      const responseText = await response.text();
      console.error(
        "Evolution API retornou resposta não-JSON ao buscar QR Code:",
        {
          status: response.status,
          responsePreview: responseText.substring(0, 200) + "...",
        }
...
export async function deleteEvolutionInstance(instanceName: string): Promise<{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`Falha ao deletar da Evolution API: ${errorMessage}`);
      // Continuar para deletar do banco mesmo se falhar na API
    } else {
    ...
    if (dbError) {
      ...
      return {
        success: false,
        error: "Erro ao deletar conexão do banco de dados",
      };
    }

    return {
      success: true,
      evolutionApiDeleted: evolutionApiSuccess,
    };
  } catch (error: any) {
