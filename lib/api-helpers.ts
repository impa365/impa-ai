...
export async function getDefaultModel(): Promise<string | null> {
  try {
    ...
    console.log("- SUPABASE_ANON_KEY:", supabaseKey ? "✅ Definida" : "❌ Não encontrada")

    if (!supabaseUrl || !supabaseKey) {
      console.error("❌ [getDefaultModel] Variáveis do Supabase não configuradas")
    ...
    console.log("🔗 [getDefaultModel] Cliente Supabase criado, fazendo query...")
    ...
    if (!data || !data.setting_value) {
      console.warn("⚠️ [getDefaultModel] default_model não encontrado no banco")
      return "gpt-4o-mini" // Fallback padrão
    }
