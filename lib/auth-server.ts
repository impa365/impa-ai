  ...
  try {
    ...
    if (request) {
      ...
      if (token) {
        try {
          ...
          console.log("✅ Usuário autenticado via JWT (header):", jwtPayload.email)
          logJWTOperation('VERIFY', jwtPayload.email, true, 'Header Authorization')
          ...
          logJWTOperation('VERIFY', 'unknown', false, `Header: ${(jwtError as Error).message}`)
        }
      }
    }
    ...
    if (jwtCookie) {
      try {
        ...
        console.log("✅ Usuário autenticado via JWT (cookie):", jwtPayload.email)
        ...
        logJWTOperation('VERIFY', 'unknown', false, `Cookie: ${(jwtError as Error).message}`)
      }
    }
